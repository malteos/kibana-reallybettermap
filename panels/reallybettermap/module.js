/** @scratch /panels/5
 *
 * include::panels/reallybettermap.asciidoc[]
 */

/** @scratch /panels/reallybettermap/0
 *
 * == reallybettermap
 * Status: *Experimental*
 *
 * reallybettermap is called reallybettermap for lack of a better name. reallybettermap uses geographic coordinates to
 ...
 *
 * IMPORTANT: reallybettermap requires an internet connection to download its map panels.
 */
define([
  'angular',
  'app',
...
function (angular, app, _, L, localRequire) {
  ...
  var module = angular.module('kibana.panels.reallybettermap', []);
  app.useModule(module);

  module.controller('reallybettermap', function($scope, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      editorTabs : [
        {
          title: 'Queries',
    ...
    var _d = {
      /** @scratch /panels/reallybettermap/3
       *
       * === Parameters
       *
       ...
       */
      field   : null,
      /** @scratch /panels/reallybettermap/5
    ...
    $scope.get_data = function(segment,query_id) {
      $scope.require(['./leaflet/plugins'], function () {
        ...
        var request = $scope.ejs.Request().indices(dashboard.indices[_segment])
          ...
          .fields([
          		$scope.panel.field + ".lat",
          		$scope.panel.field + ".lng",
          		$scope.panel.tooltip
          		])
        ...
        results.then(function(results) {
          ...
          if($scope.query_id === query_id) {
            ...
            $scope.data = $scope.data.slice(0,$scope.panel.size).concat(_.map(results.hits.hits, function(hit) {
            	...
            	var lats = hit.fields[$scope.panel.field + ".lat"];
            	var lngs = hit.fields[$scope.panel.field + ".lng"];
            	var coordinates = [];
            	for(var i in lats) {
            		coordinates[i] = new L.LatLng(lats[i], lngs[i]);
            	}
              return {
  ...
  module.directive('reallybettermap', function() {
    return {
      restrict: 'A',
      link: function(scope, elem) {
        ...
        function render_panel() {
          ...
          scope.require(['./leaflet/plugins'], function () {
            scope.panelMeta.loading = false;
            L.Icon.Default.imagePath = 'app/panels/reallybettermap/leaflet/images';
            if(_.isUndefined(map)) {
              map = L.map(scope.$id, {
            ...
            for(i in p.coordinates) {	
            	var latlng = p.coordinates[i];
            	if(!_.isUndefined(p.tooltip) && p.tooltip !== '') {
            	    markerList.push(L.marker(latlng).bindLabel(_.isArray(p.tooltip) ? p.tooltip[0] : p.tooltip));
            	  } else {
                	markerList.push(L.marker(latng));
              		}              		
              }
            });
